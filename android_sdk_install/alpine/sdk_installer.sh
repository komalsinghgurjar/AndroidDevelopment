...
##LatestSDK_URL="https://dl.google.com/android/repository/commandlinetools-linux-11076708_latest.zip"
...
# alpine ash doesn't support -P option 
SDK_VERSION=$(echo "$SDK_ZIP_FILENAME" | grep -o '[0-9]\+')

# Get the current directory
    CURRENT_DIR=$(pwd)

    # Source the set_env.sh script from the current directory
    source "$CURRENT_DIR/alpine_env_var_helper.sh"
...
set_android_home_env_var() {
    ...
    # Optionally, check the updated .profile
    echo "* .profile content during HOME env var:"
    cat "$HOME/.profile"
...
PATH=$(echo "$PATH" | tr ':' '\n' | grep -v "cmdline-tools/latest/bin" | tr '\n' ':')
   ...
   ## fi
    ...
    # Prepare the new PATH export line for .profile
    PROFILE_ENTRY="export PATH=$PATH"

# Remove all PATH entries in .profile
sed -i '/^export PATH=/d' "$HOME/.profile"
 ##sed -i '/^export PATH=.*platform-tools\/latest/d' "$HOME/.profile"

    # Append the new PATH export line to .profile
    echo "$PROFILE_ENTRY" >> "$HOME/.profile"

    # Source the updated .profile to set the new PATH for the current session
    source "$HOME/.profile"
...
echo "* bootstraping sdkmanager"
echo "* updating sdkmanager"
yes | sdkmanager --update
## to see latest platform tools and build tools
yes | sdkmanager --list
echo "* accepting sdkmanager licenses"
yes | sdkmanager --licenses
yes | sdkmanager --licenses
#yes | sdkmanager --licenses
#yes | sdkmanager --licenses
echo "* installing platform tools for android-35"
sdkmanager "platform-tools" "platforms;android-35"
echo "* installing build tools for android-35"
sdkmanager "build-tools;35.0.0"
echo "* running sdkmanager command"
sdkmanager
echo "* use sdkmanager command to manage sdk"
